...
const App = {
  setup() {
    ...
    const workerForJob = (job: Job) => dashboard.value.worker?.workers?.find((worker: any) => worker.job_id === job.id)
    const displayStatus = (job: Job) => {
      if (job.status === 'queued') return '等待 GPU Worker'
      const worker = workerForJob(job)
      if (job.status === 'running' && worker?.phase) return worker.phase
      if (job.status === 'running') return '正在执行任务'
