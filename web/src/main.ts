...
const jobStatusText = (job: Job) => {
  if (job.status === 'queued') return `${taskTypeText[job.task_type] || '任务'}排队中`
  if (job.status === 'running') return `${taskTypeText[job.task_type] || '任务'}中`
