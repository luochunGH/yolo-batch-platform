...
const App = {
  setup() {
    ...
    const workerRuns = computed(() => {
      const workers = dashboard.value.worker?.workers?.length ? dashboard.value.worker.workers : [dashboard.value.worker]
      return workers.filter(Boolean).map((worker: any, index: number) => ({
        label: `Worker ${index + 1}`,
        worker,
        job: jobs.value.find(job => job.id === worker.job_id),
      }))
    })
